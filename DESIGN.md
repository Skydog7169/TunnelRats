...
- **Ground conditions / mud (amendment, 2026-07-15):** per-material `walkFactor` movement modifiers plus a WETNESS layer — earth near water pockets is mud (slow, and in Phase 3, loud sloshing); rubble-strewn floors slow you; deep clay drags. Speed inhibitors make water pockets tactical terrain rather than mere walls, and mud IS the Western Front. Build alongside collapses (this phase) with the noise hooks left for Phase 3.
...
- **[done] Deliberate gait + pick-in-hand (2026-07-15):** r9 had silently DOUBLED the walk-cycle cadence (phase advances per tile; tiles halved) — the "sprinting" look; cadence restored and pushed ~20% longer-strided with a slight marching lean and lower, longer steps: a loaded-down trudge. And the pick is now an ITEM YOU HOLD: keys 1–4 select the active loadout slot (hotbar HUD), digging requires the pick in hand, the model only carries the pick when it's selected (empty hands otherwise). activeSlot is hashed sim state; session format gains selectSlot in flags bits 6–8 (shape-compatible — old recordings stay valid). First brick of the armorer economy: holding a weapon will mean NOT holding the pick.
